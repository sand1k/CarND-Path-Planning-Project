...
public:
  ...
  Vehicle() {}
  Vehicle(double s, double d, double speed, double accel, State state, int id);
  ...
  Trajectory choose_next_state(int tgt_lane, const Predictions &predictions);
...
private:
  vector<State> successor_states(int tgt_lane);
  ...
  Trajectory prep_lane_change_trajectory(State state, const Predictions &predictions);
